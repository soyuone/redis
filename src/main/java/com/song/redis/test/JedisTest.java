...
public class JedisTest {
	...
	public void testString() {
		...
		// 在原来key对应的值后追加一个字符串；如果key不存在，则创建一个新的key
		...
		// 保存带有效期的key，单位为秒
		jedis.setex("expir_key2", 10, "test");

	}

	/**
	 * 集合 1> 元素没有顺序 2> 元素不可重复
	 */
	...
	public void testSet() {
		...
		System.out.println(jedis.spop("hobbys"));
	...
	public void testSortedSet() {
		...
		/**
		 * 模拟搜索 区间必须包含(、[符号：（代表不包含，[代表包含 -、+ ： 代表最少和最大范围
		 * 
	...
	public void testKey() {
		...
		// /**
		// * 移动一个key到指定主机指定的数据库 注意：如果目标主机数据库中的key已经存在，则报错 arg1: 目标主机IP arg2: 目标主机端口 arg3：当前实例的key arg4: 目标主机的数据库序号 arg5:
		// * 移动超时时长
		// */
		// String s = jedis.migrate("localhost", 6379, "age", 1, 5000);
		// Assert.assertEquals(s, "OK");
		...
		System.out.println(r);
		String s = jedis.objectEncoding("height"); // 查看key在redis内部存储的数据类型
		System.out.println(s);
